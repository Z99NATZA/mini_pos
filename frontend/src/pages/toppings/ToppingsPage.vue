...
import type { Topping, Pagination } from '@/types'
...
const editing = ref<Topping | null>(null)
...
async function fetchToppings() {
  loading.value = true
  try {
    const res = await toppingService.getAll(pagination.value.page, pagination.value.per_page)
...
function openEdit(topping: Topping) {
  editing.value = topping
  form.value = { name: topping.name, price: topping.price }
  showModal.value = true
}

async function handleSubmit() {
  if (!form.value.name.trim()) {
    alert.error('กรุณากรอกชื่อท็อปปิ้ง')
    return
  }
  ...
  try {
    if (editing.value) {
      await toppingService.update(editing.value.id, { name: form.value.name.trim(), price: form.value.price })
      alert.success('แก้ไขท็อปปิ้งสำเร็จ')
    } else {
      await toppingService.create({ name: form.value.name.trim(), price: form.value.price })
      alert.success('เพิ่มท็อปปิ้งสำเร็จ')
    }
...
function handleDelete(topping: Topping) {
  alert.confirm(
    `ต้องการลบท็อปปิ้ง "${topping.name}" หรือไม่?`,
    async () => {
      try {
        await toppingService.delete(topping.id)
        alert.success('ลบท็อปปิ้งสำเร็จ')
        fetchToppings()
      } catch (err: unknown) {
        const message =
          (err as { response?: { data?: { message?: string } } })?.response?.data?.message ||
...
<template>
  <div class="p-6 space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-semibold text-(--color-text)">จัดการท็อปปิ้ง</h2>
    ...
    <AppTable>
      <thead>
        <tr class="bg-(--color-bg)">
          ...
          <th class="px-4 py-3 text-left text-xs font-medium text-(--color-text-muted) uppercase tracking-wider">ชื่อท็อปปิ้ง</th>
      ...
      <tbody>
        ...
        <tr v-else-if="toppings.length === 0">
          <td colspan="4" class="px-4 py-10 text-center text-(--color-text-muted) text-sm">ยังไม่มีข้อมูลท็อปปิ้ง</td>
        </tr>
        <tr
        ...
        >
          ...
          <td class="px-4 py-3 font-medium text-(--color-text)">{{ topping.name }}</td>
          <td class="px-4 py-3">
            <span v-if="topping.price > 0" class="text-(--color-primary) font-medium">+฿{{ formatMoney(topping.price) }}</span>
            <span v-else class="text-(--color-text-muted) text-sm">ฟรี</span>
          </td>
          <td class="px-4 py-3">
            <div class="flex items-center justify-end gap-1.5">
              <button
                @click="openEdit(topping)"
    ...
    <AppModal
      v-model="showModal"
      :title="editing ? 'แก้ไขท็อปปิ้ง' : 'เพิ่มท็อปปิ้ง'"
      size="sm"
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-(--color-text) mb-1.5">
            ชื่อท็อปปิ้ง <span class="text-(--color-error)">*</span>
          </label>
          <input
            v-model="form.name"
            type="text"
            required
            placeholder="เช่น วิปครีม, ไข่มุก, เยลลี่"
