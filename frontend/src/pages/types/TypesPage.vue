...
import type { Type, Pagination } from '@/types'
...
const editing = ref<Type | null>(null)
...
async function fetchTypes() {
  loading.value = true
  try {
    const res = await typeService.getAll(pagination.value.page, pagination.value.per_page)
...
function openEdit(type: Type) {
  editing.value = type
  form.value = { name: type.name, price: type.price }
  showModal.value = true
}

async function handleSubmit() {
  if (!form.value.name.trim()) {
    alert.error('กรุณากรอกชื่อประเภท')
    return
  }
  ...
  try {
    if (editing.value) {
      await typeService.update(editing.value.id, { name: form.value.name.trim(), price: form.value.price })
      alert.success('แก้ไขประเภทสำเร็จ')
    } else {
      await typeService.create({ name: form.value.name.trim(), price: form.value.price })
      alert.success('เพิ่มประเภทสำเร็จ')
    }
...
function handleDelete(type: Type) {
  alert.confirm(
    `ต้องการลบประเภท "${type.name}" หรือไม่?`,
    async () => {
      try {
        await typeService.delete(type.id)
        alert.success('ลบประเภทสำเร็จ')
        fetchTypes()
      } catch (err: unknown) {
        const message =
          (err as { response?: { data?: { message?: string } } })?.response?.data?.message ||
...
<template>
  <div class="p-6 space-y-4">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-semibold text-(--color-text)">จัดการประเภท</h2>
    ...
    <AppTable>
      <thead>
        <tr class="bg-(--color-bg)">
          ...
          <th class="px-4 py-3 text-left text-xs font-medium text-(--color-text-muted) uppercase tracking-wider">ชื่อประเภท</th>
      ...
      <tbody>
        ...
        <tr v-else-if="types.length === 0">
          <td colspan="4" class="px-4 py-10 text-center text-(--color-text-muted) text-sm">ยังไม่มีข้อมูลประเภท</td>
        </tr>
        <tr
        ...
        >
          ...
          <td class="px-4 py-3 font-medium text-(--color-text)">{{ type.name }}</td>
          <td class="px-4 py-3">
            <span v-if="type.price > 0" class="text-(--color-primary) font-medium">+฿{{ formatMoney(type.price) }}</span>
    ...
    <AppModal
      v-model="showModal"
      :title="editing ? 'แก้ไขประเภท' : 'เพิ่มประเภท'"
      size="sm"
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-(--color-text) mb-1.5">
            ชื่อประเภท <span class="text-(--color-error)">*</span>
          </label>
          <input
            v-model="form.name"
            type="text"
